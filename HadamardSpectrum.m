...
n = 7;         % Base parameter for Hadamard matrix size 
N = 2^n;
...
F = H * f * H;
F = log(abs(F) + 1);% better visualitation 
F = F / max(max(F));%
m = min(min(F))
...
axis image
colormap jet
c = colorbar('Ticks',[m,1], 'TickLabels',{'0','1'},FontSize = 12);
c.Label.String = 'Normalized power';
