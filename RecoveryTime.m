...
Ordered = 'walsh';  % opts = 'kronecker', 'sylvester', 'walsh', 'paley'
M = Ordered; 
% addpath('functions\');
...
HT1 = hadamard_transform_method1(f,M); % Hadamard Transform 
I = zeros(N,N,p);
...
for i = 1 : p
    ...
    % time_method_1(i) = time_method(@() inverse_hadamard_transform_method1(HT1_temp, M), num_trials);
    [I(:,:,i), time1(i)] = inverse_hadamard_transform_method1(HT1_temp, M, num_trials);
end
...
for i = 1 : p
    ...
    xticks([])
    yticks([])
    xlabel(sprintf('%4.2f %s', time1(i) * (10^(3 * pfx_num)), pfx),'Interpreter', 'tex');
end 
subplot(2,p,[i+1:2*p])
...
[pfx, pfx_num] = getTimePrefix( mean(time3) );
...
for i = 1 : p
    ...
    imagesc(I(:,:,i)), axis image, colormap gray
    title([ num2str(percentage(i)), '%' ] )
    xlabel(sprintf('%4.2f %s', time4(i) * (10^(3 * pfx_num)), pfx),'Interpreter', 'tex');
end 
...
%%
figure
semilogy(percentage, time1, LineWidth=2)
xlabel('%')
ylabel('time (s)')
grid on
hold on 
semilogy(percentage,time2,LineWidth=2)
